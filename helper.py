...
def medal_tally(df):
    meadal_tally=df.drop_duplicates(subset=['Team','NOC','Games','City','Sport','Event','Medal'])
    medal_tally=meadal_tally.groupby('region').sum()[['Gold','Silver','Bronze']].sort_values('Gold',ascending=False).reset_index()
    medal_tally['total'] = medal_tally['Gold'] + medal_tally['Silver'] + medal_tally['Bronze']

    #converting the medal counts as integer
...
def participating_nations_over_time(df):
    ...
    return year_vs_nation

def total_events_over_the_years(df):
    ...
    return year_vs_event

def total_athletes_over_the_years(df):
